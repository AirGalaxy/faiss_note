...
注意这里的``fvec_madd``，记``ivfpq.precomputed_table.data() + key * pq.ksub * pq.M`` 为precomputed_table, 在预计算距离表的章节中，我们说过precomputed_table存放的$$||Y_r||^2 +2(Y_c \cdot Y_r) $$。这里sim_table_2 存放的是$$x \cdot Y_r$$，那么``fvec_madd``实际上在计算:

$$sim\_table = precomputed\_table - 2 * sim\_table\_2 = ||Y_r||^2 +2(Y_c \cdot Y_r)  - 2 * x \cdot Y_r$$

可以看到这里已经将我们最开始计算的距离中的三项中的两项已经计算出来了